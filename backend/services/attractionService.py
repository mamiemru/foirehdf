...
def create_attraction(attraction_dict: dict) -> AttractionDTO:
    """ Create an attraction """
    
    manufacturer_name = attraction_dict.get('manufacturer')
    manufacturer: Optional[Manufacturer] = exists_manufacturer_by_name(manufacturer_name)
    
    if not manufacturer:
        raise KeyError(f"No manufacturer found with name '{manufacturer_name}'")
    ...
    attraction_payload = {
        ...
        "manufacturer_page_url": attraction_dict.get('manufacturer_page_url', None),
        "owner": attraction_dict['owner'],
        "news_page_url": attraction_dict.get('news_page_url', None),
        "videos_url": attraction_dict.get('videos_url', None),
        "images_url": attraction_dict.get('images_url', None),
    }
    ...
    raise KeyError("Failed to save new attraction")
...
def update_attraction(id: str, attraction_dict: dict) -> AttractionDTO:
    ...
    if not result:
        raise KeyError("Attraction not found")
    ...
    if success:
        return attraction_to_dto(validated_attraction)
    raise KeyError("Failed to update new attraction")
