...
def fair_edit(fair_id: str) -> None:
    ...
    with ui.row().classes("w-full flex-wrap items-start gap-6"):
        with ui.column().classes("w-full  md:w-5/12"):
            ui.label(field_value("FAIR_EDIT_FAIR")).classes("text-xl font-semibold")
            ...
            ui.input(label=field_value("FAIR_WALKTOUR_VIDEO"), placeholder="http://...").bind_value(fair_edit, "walk_tour_video").classes("w-full")

            ui.label(field_value("FAIR_SOURCES"))
            ui.input(label=field_value("FAIR_AD_URL"), placeholder="http://..." ).bind_value(fair_edit, "official_ad_page").classes("w-full")
            ui.input(label=field_value("FAIR_FACEBOOK_EVENT_PAGE_URL"), placeholder="http://...").bind_value(fair_edit, "facebook_event_page").classes("w-full")
            ui.input(label=field_value("FAIR_CITY_PAGE"), placeholder="http://...").bind_value(fair_edit, "city_event_page").classes("w-full")

            ui.button(field_value("SUBMIT"), on_click=lambda: submit_edit_fair(fair_id=fair.id, fair_edit=fair_edit))
